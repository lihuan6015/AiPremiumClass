...
# 构建dataloader
def generate_dataloader(vocab, batch_size,comments_data):
    ...
    dataloader = DataLoader(comments_data, batch_size=batch_size, shuffle=True,
                                collate_fn=convert_data)
    return dataloader


def model_train(dataloader, model, optimizer, vocab, device):
    # 训练模型
    num_epochs = 1
...
def model_eval(model, vocab, device):
    # 测试模型
    ...
    # 加载模型
    model = Comments_Classifier(len(vocab) + 2, embedding_dim, hidden_size, num_classes)
...
if __name__ == '__main__':
    ...
    # 加载训练语料
    with open('data/comments2.pkl', 'rb') as f:
    ...
    dataloader=generate_dataloader(vocab, 128,comments_data)
    # 构建模型
    # vocab_size: 词汇表大小
    # embedding_dim: 词嵌入维度
    ...
    model_train(dataloader, model, optimizer, vocab, device)

    # model_eval(model, vocab, device)